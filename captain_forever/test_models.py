...
GameObject_collide_cases = [
    ...
    (
        GameObject(
            test_object.position, load_sprite("ship", True, True), Vector2(0)
        ),
        True,
    ),
    ...
    (
        Ship(
            test_object.position, test_game.bullets.append, "ship", True, True
        ),
        True,
    ),
    ...
    (
        StaticObject(
            test_object.position + Vector2(test_object.radius) * 10, "fire"
        ),
        False,
    ),
    (
        Bullet(
            test_object.position + Vector2(test_object.radius) * 10, Vector2(0)
        ),
        False,
    ),
    (
        Ship(
            test_object.position + Vector2(test_object.radius) * 10,
    ...
    (
        StaticObject(
            test_object.position + Vector2(test_object.radius), "fire"
        ),
        True,
    ),
    (
        Bullet(test_object.position + Vector2(test_object.radius), Vector2(0)),
        False,
    ),
    (
        Ship(
            test_object.position + Vector2(test_object.radius),
            test_game.bullets.append,
            "ship",
            True,
            True,
...
ship_shoot_cases = [
    # Check that ship shoot method appends a bullet object to the game bullets
...
npc_move_shoot_cases = [
    # check to see whether the NPC shoots on move at correct cases (should only shoot with
    # heading towards player)
    ((-1, -1), 0),
...
# testing whether the move method of NPC's rotates the direction in appropriate cases
# (checks method flag)
...
@pytest.mark.parametrize(
    "clockwise_bool, expected_change", ship_rotate_degrees_cases
)
...
@pytest.mark.parametrize(
    "acceleration_value, expected_change", ship_accelerate_decellerate_cases
)
...
@pytest.mark.parametrize(
    "times_to_reduce, expected_health", ship_reduce_health_cases
)
...
def test_npc_move_shoot_cases(heading, shots):
    """
    Check that NPC shoots on move method call at correct cases (should only shoot with
    heading towards player, 3 degree variance).

    For these tests that the test_game.test_ship (player ship) is at (400, 400)
    so a ship at (0, 0) with a direction (heading) of (1, 1) is pointed DIRECTLY towards
    the player ship.
...
def test_npc_move_rotate_cases(heading, move_flag):
    """
    Check that NPC rotates on move method call at correct cases (should only rotate
    with heading not towards player, 3 deg variance).

    For these tests that the test_game.test_ship (player ship) is at
    (400, 400) so a ship at (0, 0) with a direction (heading) of (1, 1) is pointed
...
@pytest.mark.parametrize(
    "heading, npc_position, velocity_change", npc_move_velocity_cases
)
def test_npc_move_velocity_cases(heading, npc_position, velocity_change):
    """
    Check that NPC changes velocity on move method call at correct cases (should only modify
    velocity with heading toward player, and outside of a 150 and 300 pixels away window.

    For these tests that the test_game.test_ship (player ship) is at (400, 400) so a ship at
